...
`<folder>` is the locale code: `en` (source) plus `ar de es fi fr id it ja ko nl pt ru sv vi zh_CN zh_TW` — 16 targets, 17 total. The list lives in [`src/translate.constant.mjs`](src/translate.constant.mjs), which maps the Google Translate language code to the folder name (e.g. `zh-cn` → `zh_CN`).
